...
export const generateReportPdf = onRequest({ cors:true, maxInstances:10 }, async (req:Request,res:Response):Promise<void> => {
  ...
  try{
    ...
    y=H-130;
      ...
      /** KPI cards */
      drawTxt(page,"Key Performance Indicators",40,y,fontB,12,colors.black); y-=15;
      const cardW=(W-80)/3;
      const drawKpi=(title:string,val:string,col:number,row:number)=>{
        const x=40+col*(cardW+10); const y0=y-row*65;
        page.drawRectangle({x,y:y0-55,width:cardW,height:55,color:colors.lightGrey,borderColor:rgb(0.8,0.8,0.8),borderWidth:0.5});
        drawTxt(page,title,x+8,y0-20,fontR,8,colors.grey);
        drawTxt(page,val,x+8,y0-38,fontB,16,colors.black);
      };
    ...
    /** Detailed Results */
      if(Array.isArray(data.tastings)&&data.tastings.length){
        ensureSpace(100);
        drawTxt(page,"Detailed Results",40,y,fontB,12,colors.black); y-=14;
        const hdr=["#","Store","Date","Time","City","SKU A","Sold A","SKU B","Sold B","SKU C","Sold C","SKU D","Sold D","Sampled","Total Sold","Conv.%"];
        const w=[25,90,60,40,60,60,20,60,20,60,20,60,20,45,45,40];
        let x=40;
        hdr.forEach((h,i)=>{ page.drawRectangle({x,y:y-14,width:w[i],height:14,color:colors.lightGrey}); drawTxt(page,h,x+2,y-4,fontB,6,colors.black); x+=w[i]; });
        y-=14;
        for(const t of data.tastings){
          ensureSpace(12); x=40;
          const cells=[
            t.tastingNumber,t.store,fmtDate(t.scheduledDate),t.scheduledTime,t.city,
            t.skuA,t.soldA,t.skuB,t.soldB,t.skuC,t.soldC,t.skuD,t.soldD,
            t.totalConsumersSampledR,t.totalSales,
            (typeof t.conversion==='number'?(t.conversion*100).toFixed(1)+'%':'-')
          ];
          cells.forEach((c,i)=>{ drawTxt(page,String(c??'-'),x+2,y-10,fontR,6,colors.black); x+=w[i]; });
          y-=12;
        }
      }
